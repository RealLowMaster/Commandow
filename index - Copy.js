...
const FLAGS_REGEX = /^(([-])?([-])?([a-zA-Z]+)(([-][a-zA-Z]+)+)?)$/
const OPTION_TYPES = ["string", "boolean", "number"]

function CheckOptions(t, n, v) {
	if (typeof t == 'string') {
...
class Command {
	...
	add_option(t, n, v, s, d) {
		...
		s = s || null
		if (s != null) this.setOptionShortName(i, s)
		d = d || null
		if (d != null) this.setOptionDescription(i, d)
		return i;
	}

	setOptionShortName(i, s) {
		if (typeof i == 'string') {
			i = this.#o.indexOf(i)
			if (i < 0)
